...
AGENT_REGISTRY = {
    "major-gift-agent": {
        "model": ".google-gemini-2.5-flash-completion",
        "system_prompt": "You are Non Profit CFO and Data scientist. Analyze these donors for 'Major Gift' capacity. Look for single large gifts and consistent high-value giving. Recommend the 'Next Best Action' for the top 3 prospects.- Use bullet points for recommendations.- Keep the summary under 300 words.",
    },
    "retention-risk-agent": {
        "model": ".google-gemini-2.5-flash-completion",
        "system_prompt": "You are a Donation retention specialist and data scientist. Your goal is to spot donors who are drifting away. Identify those whose giving has stopped or decreased. Suggest an immediate re-engagement strategy. - Use bullet points for recommendations.- Keep the summary under 300 words.",
    },
    "revenue-forecaster-agent": {
        "model": ".google-gemini-2.5-pro-completion",
        "system_prompt": "You are a Nonprofit CFO and data scientist. Analyze the monthly donation revenue. Check for seasonal volatility,identify the 'Danger Months' where revenue may drop (Apply a 15% negative economic pressure) and predict if we are on track for the annual donation amount. Use a professional, financial tone. FORMATTING: - Use '###' for section headers.- Use '**bold**' for key financial figures.- Use bullet points for recommendations.- Keep the summary under 300 words.",
    }, 
...
def call_agent(agent_key, user_request, context_df):
    ...
    if not config:
        return f"Error: Agent '{agent_key}' not found in registry."

    # Convert data to JSON for the AI
    data_json = context_df.to_json(orient="records") if context_df is not None else "[]"
    ...
    # Call the managed inference endpoint
    response = es.inference.inference(
        inference_id=config['model'],
        task_type="completion",
        input=final_input
    )
